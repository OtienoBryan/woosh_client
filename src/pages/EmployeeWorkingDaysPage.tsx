    ...
    <div className="min-h-screen bg-gray-50">
      <div className="p-4 max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-sm font-bold text-gray-900">Employee Working Days</h1>
            <p className="text-[10px] text-gray-600 mt-0.5">Track attendance and working days for all employees</p>
          </div>
          <button
            onClick={exportToCSV}
            disabled={filteredRecords.length === 0}
            className="inline-flex items-center px-2.5 py-1 text-xs border border-transparent font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
          <div className="bg-white rounded-md p-3 shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200">
            <div className="flex items-center">
              ...
              <div className="ml-2">
                ...
                <p className="text-xs font-bold text-gray-900">{stats.totalEmployees}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-md p-3 shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200">
            <div className="flex items-center">
              ...
              <div className="ml-2">
                <p className="text-[10px] font-medium text-gray-600">Average Attendance</p>
                <p className="text-xs font-bold text-gray-900">{stats.avgAttendance.toFixed(1)}%</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-md p-3 shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200">
            <div className="flex items-center">
              ...
              <div className="ml-2">
                <p className="text-[10px] font-medium text-gray-600">Total Present Days</p>
                <p className="text-xs font-bold text-gray-900">{stats.totalPresentDays}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-md p-3 shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200">
            <div className="flex items-center">
              ...
              <div className="ml-2">
                <p className="text-[10px] font-medium text-gray-600">Total Absent Days</p>
                <p className="text-xs font-bold text-gray-900">{stats.totalAbsentDays}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-md shadow-sm border border-gray-200 p-3 mb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2.5">
            <div>
              <label className="block text-[10px] font-medium text-gray-700 mb-1">Month</label>
              <input
                type="month"
        ...
        {loading ? (
          <div className="bg-white rounded-md shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-center">
              <Clock className="h-6 w-6 text-blue-500 animate-spin mr-2" />
              <span className="text-xs text-gray-600">Loading employee data...</span>
            </div>
          </div>
        ) : error ? (
          <div className="bg-white rounded-md shadow-sm border border-red-200 p-3">
            <div className="flex items-center">
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-3 py-6 text-center">
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-[10px] text-gray-500">Try adjusting your filters or search term.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map(record => (
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-3 py-2 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-medium ${getAttendanceBadge(record.attendance_pct)}`}>
