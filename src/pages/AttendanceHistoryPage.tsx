...
const AttendanceHistoryPage: React.FC = () => {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (endDate) params.append('end_date', endDate);
      if (staffId) params.append('staff_id', staffId);
      const refreshRes = await fetch(`/api/attendance?${params.toString()}`);
  ...
  const checkMonthlyRecordCount = async (staffId: string, date: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/attendance/monthly-count?${params.toString()}`);
      if (res.ok) {
        const data = await res.json();
        setMonthlyRecordCount(data.count || 0);
      }
    } catch (error) {
  ...
  const checkExistingRecordForDate = async (staffId: string, date: string) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (data.length > 0) {
          setExistingRecordForDate(data[0]);
        } else {
          setExistingRecordForDate(null);
        }
      }
