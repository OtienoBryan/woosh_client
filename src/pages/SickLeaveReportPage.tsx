...
const SickLeaveReportPage: React.FC = () => {
  const [reportData, setReportData] = useState<SickLeaveReportItem[]>([]);
  const [filteredData, setFilteredData] = useState<SickLeaveReportItem[]>([]);
  ...
  const fetchSickLeaveReport = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await api.get('/leave-requests/sick-leave-report');
      setReportData(response.data);
      setFilteredData(response.data);
    } catch (err: any) {
      console.error('Error fetching sick leave report:', err);
      setError(err.message || 'Failed to fetch sick leave report');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `sick-leave-report-${currentYear}-${format(new Date(), 'yyyy-MM-dd')}.csv`);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="bg-white rounded-lg shadow p-4">
            <div className="text-center py-8">
              ...
              <p className="mt-3 text-xs font-medium text-gray-900">Loading sick leave report...</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-3">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-base font-bold text-gray-900">Sick Leave Report</h1>
                <p className="mt-1 text-xs text-gray-500">
                  Sick leave entitlement, taken days, and remaining balance for {currentYear}
                </p>
              </div>
...
export default SickLeaveReportPage;
