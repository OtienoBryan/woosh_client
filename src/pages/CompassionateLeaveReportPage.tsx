...
interface CompassionateLeaveReportItem {
  id: number;
  name: string;
...
const CompassionateLeaveReportPage: React.FC = () => {
  const [reportData, setReportData] = useState<CompassionateLeaveReportItem[]>([]);
  const [filteredData, setFilteredData] = useState<CompassionateLeaveReportItem[]>([]);
  ...
  const fetchCompassionateLeaveReport = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await api.get('/leave-requests/compassionate-leave-report');
      setReportData(response.data);
      setFilteredData(response.data);
    } catch (err: any) {
      console.error('Error fetching compassionate leave report:', err);
      setError(err.message || 'Failed to fetch compassionate leave report');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `compassionate-leave-report-${currentYear}-${format(new Date(), 'yyyy-MM-dd')}.csv`);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="bg-white rounded-lg shadow p-4">
            <div className="text-center py-8">
              ...
              <p className="mt-3 text-xs font-medium text-gray-900">Loading compassionate leave report...</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="bg-white rounded-lg shadow p-4">
            <div className="text-center py-8">
              ...
              <button
                onClick={fetchCompassionateLeaveReport}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-3">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-base font-bold text-gray-900">Compassionate Leave Report</h1>
                <p className="mt-1 text-xs text-gray-500">
                  Compassionate leave entitlement, taken days, and remaining balance for {currentYear}
                </p>
              </div>
...
export default CompassionateLeaveReportPage;
