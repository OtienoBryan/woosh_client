...
interface GrossMarginDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  period: string;
...
const GrossMarginDetailsModal: React.FC<GrossMarginDetailsModalProps> = ({
  isOpen,
  onClose,
  period,
  customStartDate,
  ...
  const fetchGrossMarginDetails = async () => {
    setLoading(true);
    setError(null);
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/financial/reports/gross-margin-details?${params}`);
      if (response.data.success) {
        setData(response.data.data);
      } else {
        setError(response.data.error || 'Failed to fetch gross margin details');
      }
    } catch (err: any) {
      setError(err.message || 'Failed to fetch gross margin details');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">Gross Margin Details</h2>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          {loading && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-3 text-gray-600">Loading gross margin details...</span>
            </div>
          )}
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {data.product_summary.map((product) => (
                          <tr key={product.product_id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {formatCurrency(product.total_revenue - product.total_tax)}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
...
export default GrossMarginDetailsModal;
